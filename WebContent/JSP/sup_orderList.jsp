...
<table align="center" style="width: 50%; height: 100px; margin:auto;" border="1">
        <thead>
         ...
         <tbody>
            <c:if test="${totalCount!=0}">
                <c:forEach items="${boardList}" var="list" varStatus="status">
                    <tr>
                        ...
                        <c:if test="${list.complete eq 'A'}">
                         	<td style="text-align: center;">
                            	<input type="button" value="납품하기" onclick="location.href='delivery_check.do?orderNum=${list.orderNum}&order_vol=${list.order_vol }&p_num=${list.p_num}'"/>
                        	</td>
                        </c:if>
                        
                        <c:if test="${list.complete eq 'B'}">
                        	 <td style="text-align: center;">납품 완료</td>
                        </c:if>
                    </tr>
                </c:forEach>
            </c:if>
            ...
            <tr>
            	<th colspan="7"><input type="button" value="돌아가기" onclick="location.href='sup_main.do'"/></th>
            </tr>
        </tbody>
        </table>
</body>
